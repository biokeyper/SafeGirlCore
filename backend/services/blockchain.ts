...
class BlockchainService {
  ...
  async submitReport(
    ...
    try {
      ...
      logger.logBlockchain("Submit Report", "failure", {
  ...
  async grantAccess(
    ...
    try {
      ...
      logger.logBlockchain("Grant Access", "failure", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
   ...
   * @returns {Promise<object>} Transaction details
   */
  async revokeAccess(viewerAddress: string): Promise<TransactionResult> {
    try {
      ...
      logger.logBlockchain("Revoke Access", "failure", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
   ...
   */
  async getActiveConsents(reporterAddress: string): Promise<any[]> {
    try {
   ...
   */
  async getReportStatusFromContract(userId: string | number): Promise<ReportStatus> {
    try {
      const userKey = this.getUserKey(userId);
   ...
   */
  getUserKey(userId: string | number | undefined | null): string {
   ...
   * @returns {Promise<object>} Transaction details
   */
  async sendPanicAlert(locationData: string): Promise<TransactionResult> {
    try {
      ...
      logger.logBlockchain("Send Panic Alert", "failure", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
