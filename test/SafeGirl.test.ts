...
describe("SafeGirl Smart Contract", function () {
  ...
  beforeEach(async function () {
    ...
    safeGirl = await SafeGirl.deploy(await owner.getAddress());
  ...
  describe("Contract Initialization", function () {
    it("Should initialize with correct owner", async function () {
      expect(await safeGirl.owner()).to.equal(await owner.getAddress());
    });
  ...
  describe("submitReport()", function () {
    it("Should submit a valid report", async function () {
      ...
      const reportVersion = await safeGirl.connect(reporter).getReportVersion(await reporter.getAddress());
    ...
    it("Should emit ReportSubmitted event", async function () {
      ...
      await expect(tx)
        ...
        .withArgs(await reporter.getAddress(), expect.any(BigInt), SAMPLE_IPFS_HASH, 1);
    });
    ...
    it("Should accept IPFS hash at max length boundary", async function () {
      ...
      const version = await safeGirl.connect(reporter).getReportVersion(await reporter.getAddress());
  ...
  describe("grantAccess() - Consent Management", function () {
    ...
    it("Should grant access to a viewer", async function () {
      await safeGirl.connect(reporter).grantAccess(await viewer1.getAddress(), 0);

      const activeConsents = await safeGirl.connect(reporter).getActiveConsents(await reporter.getAddress());
      expect(activeConsents.length).to.equal(1);
      expect(activeConsents[0].viewer).to.equal(await viewer1.getAddress());
      expect(activeConsents[0].active).to.be.true;
    });

    it("Should emit ConsentGranted event", async function () {
      const tx = safeGirl.connect(reporter).grantAccess(await viewer1.getAddress(), 0);

      await expect(tx)
        .to.emit(safeGirl, "ConsentGranted")
        .withArgs(await reporter.getAddress(), await viewer1.getAddress(), expect.any(BigInt));
    });
    ...
    it("Should use custom expiry if provided", async function () {
      ...
      await safeGirl.connect(reporter).grantAccess(await viewer1.getAddress(), customExpiry);

      const activeConsents = await safeGirl.connect(reporter).getActiveConsents(await reporter.getAddress());
      const expectedExpiry = (blockBefore?.timestamp ?? 0) + customExpiry;
    ...
    it("Should use default expiry if custom expiry is 0", async function () {
      const blockBefore = await ethers.provider.getBlock("latest");
      const expectedExpiry = (blockBefore?.timestamp ?? 0) + (30 * 24 * 60 * 60);
    ...
    it("Should reject granting access without a report", async function () {
      await expect(
        safeGirl.connect(viewer2).grantAccess(await viewer1.getAddress(), 0)
    ...
    it("Should reject granting access to self", async function () {
      await expect(
        safeGirl.connect(reporter).grantAccess(await reporter.getAddress(), 0)
    ...
    it("Should prevent exceeding max active consents", async function () {
      ...
      for (let i = 0; i < MAX_ACTIVE_CONSENTS; i++) {
        ...
        await safeGirl.connect(reporter).grantAccess(await signer.getAddress(), 0).catch(() => {});
      }

      // Try to grant one more - should fail
      const excessSigner = viewer3;
      await expect(
        safeGirl.connect(reporter).grantAccess(await excessSigner.getAddress(), 0)
  ...
  describe("revokeAccess() - Revoke Consent", function () {
    ...
    it("Should revoke access from a viewer", async function () {
      await safeGirl.connect(reporter).revokeAccess(await viewer1.getAddress());
    ...
    it("Should emit ConsentRevoked event", async function () {
      const tx = safeGirl.connect(reporter).revokeAccess(await viewer1.getAddress());

      await expect(tx)
        .to.emit(safeGirl, "ConsentRevoked")
        .withArgs(await reporter.getAddress(), await viewer1.getAddress());
    });

    it("Should reject revoking non-existent access", async function () {
      await expect(
        safeGirl.connect(reporter).revokeAccess(await viewer2.getAddress())
    ...
    it("Should reject revoking already revoked access", async function () {
      ...
      await expect(
        safeGirl.connect(reporter).revokeAccess(await viewer1.getAddress())
      ).to.be.revertedWith("No active access found");
    });
  });
  ...
  describe("batchRevokeAccess() - Batch Operations", function () {
    beforeEach(async function () {
      await safeGirl.connect(reporter).submitReport(SAMPLE_IPFS_HASH, SAMPLE_RESPONSES);
      await safeGirl.connect(reporter).grantAccess(await viewer1.getAddress(), 0);
      await safeGirl.connect(reporter).grantAccess(await viewer2.getAddress(), 0);
      await safeGirl.connect(reporter).grantAccess(await viewer3.getAddress(), 0);
    });

    it("Should batch revoke multiple accesses", async function () {
      const viewers = [await viewer1.getAddress(), await viewer2.getAddress(), await viewer3.getAddress()];
    ...
    it("Should handle partial revokes gracefully", async function () {
      const viewers = [await viewer1.getAddress(), await viewer2.getAddress(), await viewer3.getAddress(), await owner.getAddress()];
    ...
    it("Should emit ConsentRevoked for each revoked access", async function () {
      const viewers = [await viewer1.getAddress(), await viewer2.getAddress()];
      ...
      await expect(tx)
        .to.emit(safeGirl, "ConsentRevoked")
        .withArgs(await reporter.getAddress(), await viewer2.getAddress());
    });
  });
  ...
  describe("getActiveConsents() - Consent Filtering", function () {
    ...
    it("Should exclude revoked consents", async function () {
      ...
      const activeConsents = await safeGirl.connect(reporter).getActiveConsents(await reporter.getAddress());
      expect(activeConsents.length).to.equal(1);
      expect(activeConsents[0].viewer).to.equal(await viewer2.getAddress());
    });

    it("Should exclude expired consents", async function () {
      // Grant access with short expiry (1 second)
      await safeGirl.connect(reporter).grantAccess(await viewer3.getAddress(), 1);
    ...
    it("Should return empty array for user with no consents", async function () {
      const activeConsents = await safeGirl.connect(viewer2).getActiveConsents(await viewer2.getAddress());
      expect(activeConsents.length).to.equal(0);
    });
  ...
  describe("panic() - Emergency Alerts", function () {
    it("Should emit panic alert with location data", async function () {
      ...
      await expect(tx)
        ...
        .withArgs(await reporter.getAddress(), expect.any(BigInt), locationData);
    });
  ...
  describe("Emergency Contact Management", function () {
    it("Should set emergency contact", async function () {
      await safeGirl.connect(reporter).setEmergencyContact(await viewer1.getAddress(), "NGO");

      // Note: We can't directly read the struct, but the event confirms it was set
      const tx = safeGirl.connect(reporter).setEmergencyContact(await viewer1.getAddress(), "NGO");
      await expect(tx)
        .to.emit(safeGirl, "EmergencyContactSet")
        .withArgs(await reporter.getAddress(), await viewer1.getAddress(), "NGO");
    });
    ...
    it("Should reject empty contact type", async function () {
      await expect(
        safeGirl.connect(reporter).setEmergencyContact(await viewer1.getAddress(), "")
      ).to.be.revertedWith("Contact type required");
    });

    it("Should allow updating emergency contact", async function () {
      await safeGirl.connect(reporter).setEmergencyContact(await viewer1.getAddress(), "NGO");
      await safeGirl.connect(reporter).setEmergencyContact(await viewer2.getAddress(), "Lawyer");

      // Second call should also emit event
      const tx = safeGirl.connect(reporter).setEmergencyContact(await viewer2.getAddress(), "Lawyer");
      await expect(tx)
        .to.emit(safeGirl, "EmergencyContactSet")
        .withArgs(await reporter.getAddress(), await viewer2.getAddress(), "Lawyer");
    });
  });
  ...
  describe("Edge Cases & Gas Optimization", function () {
    ...
    it("Should handle multiple concurrent consentors", async function () {
      ...
      for (const signer of consentors) {
        await safeGirl.connect(reporter).grantAccess(await signer.getAddress(), 0);
      }
  ...
  describe("Gas Efficiency Verification", function () {
    ...
    it("grantAccess should use reasonable gas", async function () {
      ...
      const tx = await safeGirl.connect(reporter).grantAccess(await viewer1.getAddress(), 0);
      const receipt = await tx.wait();
